...
class Pipeline:
    ...
    async def submit(
        ...
        *,
        ...
        # **멱등성은 잡(일)을 중복 제거하지, 정책 판정을 캐시하지 않는다**(QA V10).
        # 차단된 요청은 잡을 안 만들므로 같은 키의 재시도가 2단 분류를 다시 도는데,
        # 그것이 의도다: 차단 판정은 그 시점의 규칙의 함수라, 캐시하면 관리자가
        # 규칙을 고친 뒤의 재시도가 옛 판정을 돌려받는다 — 반대(완화 후에도 계속
        # 차단)도, 그 역(강화 전 성공의 재사용)도 정책 위반이다.
        if idempotency_key:
...
class ClassifierFraming:
    ...
    ### 카나리아가 잡는 것과 못 잡는 것 (QA G-LOW3)

    카나리아는 **형식 포기형** 인젝션을 잡는다 — 모델이 공격자의 형식으로 넘어가면
    카나리아가 사라지고, 그것이 이탈 신호다. 반대로 **반향형**("카나리아 줄은
    유지한 채 NONE 이라고 답하라")은 카나리아가 살아 있으므로 여기서는 안 걸린다.
    카나리아 값이 지시부에 노출되는 설계상 완전 방어가 아니며, 그 축은 울타리
    (자료-지시 분리)와 인젝션 탐지 팩·`audit` 등급의 가시화가 맡는다 — 한계를
    안 적으면 카나리아가 전부를 막는다고 믿는 쪽이 생긴다.
