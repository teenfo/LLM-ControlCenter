...
BUNDLED_FIXTURES: tuple[tuple[str, str, bool], ...] = (
    # (규칙 id, 텍스트, 잡혀야 하는가)
    #
    # **조사 밀착 표본이 반드시 있어야 한다.** 한동안 이 세트의 모든 한국어 표본이
    # 조사 앞에 공백을 넣은 형태였고("…5678 입니다"), 그래서 `\b` 경계가 조사
    # 밀착 표기에서 무너지는 미탐(QA G-HIGH)이 커버리지 장치에 안 걸렸다 —
    # 코퍼스가 실사용 표기를 안 닮으면 100% 통과가 곧 측정의 부재다.
    ("kr_rrn", "주민등록번호는 900101-1234568 입니다", True),
    ("kr_rrn", "제 주민번호는 900101-1234568입니다", True),      # 조사 밀착
    ("kr_phone", "전화번호 010-1234-5678로 연락주세요", True),     # 조사 밀착
    ("credit_card", "카드번호 4111-1111-1111-1111입니다", True),  # 조사 밀착
    ("kr_biz_no", "사업자번호 123-45-67891입니다", True),         # 조사 밀착
    ("jp_mynumber", "マイナンバーは1234-5678-9018です", True),     # 일본어 밀착
    # ASCII 식별자 문맥은 여전히 안 잡는다 — 경계 클래스가 지키는 다른 절반이다.
    ("credit_card", "ref v2-4111-1111-1111-1111x 확인", False),
