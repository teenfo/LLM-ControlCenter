...
ROUTE_SUMMARIES: Mapping[str, tuple[str, str, bool]] = {
    ...
    "tenant_job_raw": (
        "프롬프트·응답 원문 단건 복호화. **열람 자체가 감사에 남는다.**",
        "tenant_admin", False,
    ),
    "tenant_job_review": (
...
def integration_guide(
    ...
    *,
    ...
    lines += [
        ...
        "",
        "`wait` 이 `pending` 을 돌려준 뒤 연결이 끊기면, 재시도하는 것이 옳다. 다만",
        "**그 재시도가 두 번째 작업을 만들면 과금도 두 번**이다. 같은 작업이라는 표시를",
        "헤더로 붙인다:",
        "",
        "```",
        "Idempotency-Key: <소비자가 정한 값>",
        "```",
        "",
        ...
        "",
        "개인정보 외에 **프롬프트 인젝션 구문**도 규칙 대상이다. 채팅 템플릿 제어 토큰",
        "(`<|im_start|>` 등)은 베이스라인에서 마스킹되므로, 그런 토큰을 본문에 담아 보내면",
        "가려진 채 전달된다 — 모델에게 역할을 지정하려면 `system` 필드를 쓴다.",
        "",
        "**자격증명도 마스킹된다.** API 키·개인키·`api_key = …` 형태의 대입은 저장 전에",
        "가려지므로, 설정 파일을 통째로 붙여 넣고 디버깅을 요청하면 값 부분이 비어서 간다.",
        "이 서비스의 토큰도 대상이다 — 자기 토큰을 프롬프트에 넣지 않는다.",
        "",
        ...
        "### 응답도 검사된다",
        "",
        "**`response` 는 마스킹본이다.** 모델이 만들어 낸 텍스트에도 같은 1단 규칙이 적용되고,",
        "걸린 값은 규칙 등급대로 가려진 뒤에야 저장·반환된다. 요약·추출 작업은 산출물 자체가",
        "개인정보일 수 있으므로 입력만 거르면 절반만 거르는 것이다.",
        "",
        "실무에서 중요한 두 가지:",
        "",
        "- **응답 문자열을 파싱해 분기하지 말 것.** 마스킹이 걸리면 모양이 바뀐다. 규칙 등급은",
        "  관리자가 언제든 조일 수 있으므로 오늘 통과한 형식이 내일 가려질 수 있다.",
        "- 차단 등급 규칙이 응답에 걸려도 **작업은 실패하지 않는다.** 값만 가려진 채 돌아온다 —",
        "  이미 추론이 끝난 뒤라 통째로 버리면 비용만 내고 아무것도 못 받기 때문이다.",
        "",
        "원문이 필요하면 테넌트 관리자가 단건 복호화 API 로 열람한다. **그 열람은 감사에 남는다.**",
        "",
        "## 7. 현재 한도",
        "",
    ]
