...
| 밖 → 안 | 클라이언트 페이지 `/client/` · 내 작업 목록 `GET /v1/jobs` | `user` 계정 로그인 → `service` 역할 세션 · `end_user` 를 아이디로 강제 · 목록 범위는 (테넌트, 서비스, 엔드유저) | TLS 종단 (**공개돼야 한다.** 사람이 브라우저로 쓴다) |
| 밖 → 안 | 플러그인 면 `/v1/plugin/*` (tick · events) | `service` 토큰 · 그 서비스가 플러그인 것이어야 한다(아니면 404) · `active_service` 통과 — 끄면 401 | — (**공개돼야 한다.** external 플러그인이 원격에서 묻는다. 설치·검사·켜기·회전은 플랫폼 면이라 아래 줄) |
